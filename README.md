...
- [ ] Create a new virtualenv with either [virtualenvwrapper](https://virtualenvwrapper.readthedocs.io/en/latest/) or only virtualenv: `mkvirtualenv Tag_Strategies` or `python -m venv Tag_Strategies-venv`
...
- [ ] Make sure the virtualenv is activated `workon Tag_Strategies` or `source Tag_Strategies-venv/bin/activate`
- [ ] Compile the requirements before installation and install them:  `make compile_install_requirements`
- [ ] Change the first line of README to the name of the project
- [ ] Add an email address to the `ADMINS` settings variable in `Tag_Strategies/backend/Tag_Strategies/settings/base.py`
- [ ] Change the `SERVER_EMAIL` to the email address used to send e-mails in `Tag_Strategies/backend/Tag_Strategies/settings/production.py`
...
- Create a copy of ``Tag_Strategies/settings/local.py.example``:  
  `cp Tag_Strategies/settings/local.py.example Tag_Strategies/settings/local.py`
...
- Deregister the example app by removing `'exampleapp.apps.ExampleappConfig'` from ``backend/Tag_Strategies/settings/base.py``.
- Adjust ``backend/Tag_Strategies/urls.py`` to point to your newly created Django app and remove the path configuration that redirects to the deleted example app.
